...
export default function DashboardView({ session, accountType }: Props) {
  ...
  // Plan gratis: forzar mes en curso si por alguna razon el state quedo en otro.
  useEffect(() => {
    if (!isFree) return
    const curY = now.getFullYear()
    const curM = now.getMonth() + 1
    if (year !== curY) setYear(curY)
    if (month !== curM) setMonth(curM)
  ...
  return (
    ...
    <main className="flex-1 flex flex-col lg:ml-60">
        ...
        <div className="border-b border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/60 px-6 py-5 backdrop-blur-sm">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {/* Navegador de mes */}
              <div className="flex items-center gap-1 rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-1 py-1" title={isFree ? 'Plan gratis: solo el mes en curso' : undefined}>
                <button onClick={prevMonth} disabled={isFree} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition text-slate-500 dark:text-zinc-400 disabled:opacity-30 disabled:cursor-not-allowed">
                ...
                <button onClick={nextMonth} disabled={isCurrentMonth || isFree} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition text-slate-500 dark:text-zinc-400 disabled:opacity-30 disabled:cursor-not-allowed">
        ...
        {isFree && (
          <div className="bg-amber-50 dark:bg-amber-950/40 border-b border-amber-200 dark:border-amber-800 px-6 py-2.5 flex items-center justify-between gap-3">
            <p className="text-xs sm:text-sm font-semibold text-amber-800 dark:text-amber-200">
              Plan gratis: solo puedes ver y descargar el mes en curso. Suscribete para desbloquear el historico de 5 anios y mas RFCs.
            </p>
            <button
