...
export default function DashboardView({ session, accountType, xmlMode = false }: Props) {
  ...
  function nextYear() {
    ...
    const nextY = year + 1
    setYear(nextY)
    // Si al subir de año caemos en el año actual y el mes quedó en el futuro,
    // fijamos el mes al mes actual (no hay datos por venir).
    if (nextY === now.getFullYear() && month > now.getMonth() + 1) {
        ...
        <div className="border-b border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/60 px-6 py-5 backdrop-blur-sm">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {/* Navegadores de mes y año (independientes) — freemium queda fijo al periodo actual salvo en modo XML */}
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1 rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-1 py-1">
                  {!restrictHistory && (
                    <button onClick={prevMonth} aria-label="Mes anterior" className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition text-slate-500 dark:text-zinc-400">
                  ...
                  <span className="px-2 text-sm font-semibold text-slate-700 dark:text-zinc-200 min-w-[92px] text-center">
                    {MESES[month - 1]}
                  </span>
                  {!restrictHistory && (
                    <button onClick={nextMonth} disabled={isCurrentMonth} aria-label="Mes siguiente" className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition text-slate-500 dark:text-zinc-400 disabled:opacity-30 disabled:cursor-not-allowed">
                ...
                <div className="flex items-center gap-1 rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-1 py-1">
                  {!restrictHistory && (
                    <button onClick={prevYear} aria-label="Año anterior" className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition text-slate-500 dark:text-zinc-400">
                      <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><polyline points="15 18 9 12 15 6"/></svg>
                    </button>
                  )}
                  <span className="px-2 text-sm font-semibold text-slate-700 dark:text-zinc-200 min-w-[52px] text-center">
                    {year}
                  </span>
                  {!restrictHistory && (
                    <button onClick={nextYear} disabled={isCurrentYear} aria-label="Año siguiente" className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition text-slate-500 dark:text-zinc-400 disabled:opacity-30 disabled:cursor-not-allowed">
                      <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>
                    </button>
                  )}
                </div>
